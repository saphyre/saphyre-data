...
function getValue(type, value) {
  ...
  const transformer = transformers[type.key];
...
function QueryBuilder(model, provider, functions, config) {
  const transaction = config && config.transaction;
  ...
  this.query = new Query(model.sequelize, transaction);
    ...
    if (_.isObject(alias)) {
      if (alias.list && alias.projection) {
        ...
        builder.postProcesses.push(item => {
          if (item != null) {
            const queryBuilder = new QueryBuilder(builder.model, builder.provider, builder.functions);
...
QueryBuilder.prototype.list = function () {
  const builder = this,
    query = this.query;
  return Promise.all(this.globalHandlers).then(() => {
...
QueryBuilder.prototype.applyPath = function (path, joinInner, force, criteria, criterias) {
  ...
  result.property = model.rawAttributes[result.field].field;
